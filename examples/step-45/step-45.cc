...
namespace Step45
{
  ...
  void StokesProblem<dim>::create_mesh()
  {
    ...
    const auto refinement_subdomain_predicate = [&](const auto &cell) {
      return (cell->center()(1) > 0.75 && cell->center()(1) < 1 &&
              cell->center()(2) < .25);
    };
  ...
  void StokesProblem<dim>::resolve_hanging_on_periodic_boundary_2()
  {
    ...
    for (auto &cell : triangulation.active_cell_iterators())
      {
        if (cell->is_ghost() && cell->at_boundary())
          {
            for (unsigned int i = 0; i < cell->n_faces(); ++i)
              {
                if (cell->has_periodic_neighbor(i) &&
  ...
  void StokesProblem<dim>::resolve_hanging_on_periodic_boundary()
  {
    ...
    recvcounts =
      (int *)malloc(ranks * sizeof(int)); // will be used to gather cell ids
    local_count = local_count * 4;
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Allgather(
      &local_count, 1, MPI_INT, recvcounts, 1, MPI_INT, MPI_COMM_WORLD);
    displs[0] = 0;
            ...
            for (unsigned int i = 0; i < cell->n_faces(); ++i)
              {
                if (cell->has_periodic_neighbor(i))
                  {
                    TriaIterator<CellAccessor<dim, dim>> temp =
            ...
            for (unsigned int i = 0; i < cell->n_faces(); ++i)
              {
                if (cell->has_periodic_neighbor(i))
                  {
                    TriaIterator<CellAccessor<dim, dim>> neighbor =
                      cell->periodic_neighbor(i);
  ...
  void StokesProblem<dim>::exchange_refinement_flags()
  {
    ...
    auto pack = [](const typename DoFHandler<dim>::active_cell_iterator &cell)
      -> std::uint8_t {
    ...
    auto unpack = [](const typename DoFHandler<dim>::active_cell_iterator &cell,
  ...
  void StokesProblem<dim>::refine_mesh()
  {
    ...
    const auto refinement_subdomain_predicate = [&](const auto &cell) {
      // return (cell->center()(0) < 0.25 && cell->center()(1) > 0.75 &&
      // cell->center()(1)< 1 && cell->center()(2)<.25);
      return (rand() % 100) < 10;
    };
    ...
    //     changed =
    //     (Utilities::MPI::max(changed?1:0,triangulation.get_communicator())==1)
    //     ? true : false;
  ...
  //   prepare();
  //   exchange();
  //   for all of my cells, if a periodic neighbor is marked refinement,
  //   set_refine_flag() and changed=true;
  ...
  void StokesProblem<dim>::run()
  {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    std::cout
      << "number of level differences across periodic boundary detected is "
      << verifier() << "\n";
  }
} // namespace Step45
